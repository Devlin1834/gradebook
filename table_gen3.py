...
def list_break(lst, n):
    ends = [i for i in range(len(lst)) if i % n == 0]
    broken = []
...
class Table():
    ...
    def pull(self, given, ask = 'Column'):
        ...
        if type(given) is int:
            ...
            sub = given.lower()
            found = {'Column': [c.data for c in contents if c.col.lower() == sub],
                    'Row': [c.data for c in contents if c.row.lower() == sub]}
    ...
    def group_order(self, subject):
        ## Handle being passed a string
        clow = [i.lower() for i in self.cnames]
        if type(subject) is str and subject.lower() in clow:
            subject = clow.index(subject.lower())
        ...
        arranged.insert(subject + 1, base)
